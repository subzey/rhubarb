...
function replace(code, modifiedNodes) {
	var right = 0;
	var chunks = [];

	for (var i = modifiedNodes.length; i--;) {
		var node = modifiedNodes[i];
		console.log(node);
		if (node.range[1] <= right) {
			continue;
		}
		chunks.push(code.substring(right, node.range[0]));
		chunks.push(_serialize(node.buildtimeComputedValue));
		right = node.range[1];
	}
...
function inline(code, state) {
	...
	for (var i = idNodes.length; i--;) {
		...
		for (var j = idNodes[i].stack.length; j--;) {
			var astNode = idNodes[i].stack[j];
	...
	return replace(code, modifiedNodes);
}
